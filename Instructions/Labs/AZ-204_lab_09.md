...
1. 다음 명령을 실행하여 NuGet에서 **Azure.Messaging.EventGrid** 버전 4.10.0을 가져옵니다.
    ...
    > **참고**: **dotnet add package** 명령은 NuGet에서 **Microsoft.ApplicationInsights.PerfCounterCollector** 패키지를 추가합니다. 더 자세한 내용은 [Azure.Messaging.EventGrid](https://www.nuget.org/packages/Azure.Messaging.EventGrid/4.10.0)를 참조하세요.

    > 다음 명령을 실행하여 .NET 웹 애플리케이션을 빌드합니다. **터미널 종료** 또는 **휴지통** 아이콘을 선택하여 현재 열려 있는 터미널 및 관련된 모든 작업을 종료합니다.


1. 작업 2: Program 클래스를 수정하여 Event Grid에 연결
...
#### <a name="task-2-modify-the-program-class-to-connect-to-event-grid"></a> **Program.cs** 파일의 코드 편집기 탭에서 기존 파일의 모든 코드를 삭제합니다.
...
1. 다음 코드 줄을 추가하여 **Uri** 형식의 새 변수 **endpoint**를 만듭니다. 이때 생성자 매개 변수로는 **topicEndpoint** 문자열 상수를 사용합니다.
    ...
    **Main** 메서드를 검토합니다. 이제 메서드에 다음 코드가 포함되어 있어야 합니다.  **Program.cs** 파일을 저장합니다.
...
1. **참고**: 빌드 오류가 있는 경우, **Allfiles (F):\\Allfiles\\Labs\\09\\Solution\\EventPublisher** 폴더에 있는 **Program.cs** 파일을 검토하세요.
...
1. 현재 실행 중인 콘솔 애플리케이션의 성공 메시지 출력을 관찰합니다.
    ...
    > **터미널 종료** 또는 **휴지통** 아이콘을 선택하여 현재 열려 있는 터미널 및 관련된 모든 작업을 종료합니다.

1. 작업 4: 게시된 이벤트 살펴보기

1. **Azure Event Grid 뷰어** 웹 애플리케이션이 있는 브라우저 창으로 돌아갑니다.

#### <a name="task-4-observe-published-events"></a>콘솔 애플리케이션으로 생성한 **Employees.Registration.New** 이벤트를 검토합니다.
...
1. 이 연습에서는 .NET 콘솔 애플리케이션을 사용하여 Event Grid 토픽에 새 이벤트를 게시했습니다.

#### <a name="review"></a>연습 4: 구독 정리

작업 1: Azure Cloud Shell 열기

### <a name="exercise-4-clean-up-your-subscription"></a>Azure Portal에서 **Cloud Shell** 아이콘 ![Cloud Shell 아이콘](./media/az204_lab_CloudShell.png)을 선택하여 새 Bash 세션을 엽니다.

#### <a name="task-1-open-azure-cloud-shell"></a>Cloud Shell 기본값이 PowerShell 세션인 경우 **PowerShell**을 선택한 다음, 드롭다운 메뉴에서 **Bash**를 선택합니다.

1. **참고**: **Cloud Shell**을 처음 시작하는 경우 **Bash** 또는 **PowerShell**을 선택하라는 메시지가 표시되면 **PowerShell**을 선택합니다. **탑재된 스토리지가 없음** 메시지가 표시되면 이 랩에서 사용하는 구독을 선택하고 **스토리지 만들기**를 선택합니다.

    > 작업 2: 리소스 그룹 삭제 **Cloud Shell** 창에서 다음 명령을 실행하여 **PubSubEvents** 리소스 그룹을 삭제합니다.

#### <a name="task-2-delete-a-resource-group"></a>**참고**: 이 명령은 *--no-wait* 매개 변수에서 알 수 있듯이 비동기로 실행되므로 동일한 Bash 세션 내에서 이 명령을 실행한 직후에 다른 Azure CLI 명령을 실행하는 것이 가능하지만 실제로 리소스 그룹이 제거되기까지는 몇 분 정도 걸립니다.
...
#### <a name="task-3-close-the-active-applications"></a>현재 실행 중인 Visual Studio Code 애플리케이션을 닫습니다.

1. 검토

1. 이 연습에서는 이 랩에 사용된 리소스 그룹을 제거하여 구독을 정리했습니다.
