...
1. 고급 탭의 ****이벤트 스키마 드롭다운 목록에서 Event Grid 스키마**** 항목이 선택되어 있는지 확인하고 **검토 + 만들기**를 선택합니다**.**
...
1. **이벤트 구독 만들기** 창에서 다음 작업을 수행한 후, **만들기**를 선택합니다.
    ...
    | **엔드포인트** | 엔드포인트** 구성을 선택합니다**. **구독자 엔드포인트** 텍스트 상자에 이전에 기록한 **웹앱 URL** 값을 입력하고, 이것이 **https://** 접두사가 사용되는지 확인하고 **/api/updates** 접미사를 추가한 후에 **선택 확인**을 선택합니다. 예를 들어, **웹앱 URL** 값이 ``http://eventviewerstudent.azurewebsites.net/``이면 **구독자 엔드포인트**가 ``https://eventviewerstudent.azurewebsites.net/api/updates``가 됩니다. |
...
1. 다음 코드를 추가합니다.
    ...
    ```
1. 7줄에서 자리 표시자를 이 랩의 앞부분에서 기록한 Event Grid 토픽 엔드포인트의 값으로 바꿉 `<topic-endpoint>` 다.

1. 10줄에서 자리 표시자를 이 랩의 앞부분에서 기록한 Event Grid 토픽 액세스 키 값으로 바꿉 `<topic-key>` 다.
