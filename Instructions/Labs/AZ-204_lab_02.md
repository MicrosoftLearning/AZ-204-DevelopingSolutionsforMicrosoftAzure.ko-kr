...
1. **Echo.cs** 파일 내의 모든 내용을 삭제하고 다음 코드를 추가합니다. 이것은 POST 요청을 받아서 요청 본문을 OkObjectResult로 반환하는 “Echo”라는 HTTP 트리거 함수를 정의합니다. ILogger 인터페이스는 정보 로깅에도 사용됩니다. 이 함수는 HTTP 요청 및 응답을 테스트하는 데 사용할 수 있습니다.
...
1. **GetSettingInfo.cs** 파일 내의 모든 내용을 삭제하고 다음 코드를 추가합니다. 이 코드는 HTTP GET 요청에 대한 응답으로 Azure Blob Storage 컨테이너에 있는 JSON 파일의 내용을 반환합니다.
...
5. **함수 앱** 창의 **함수** 섹션에서 **함수** 옵션을 선택합니다.
