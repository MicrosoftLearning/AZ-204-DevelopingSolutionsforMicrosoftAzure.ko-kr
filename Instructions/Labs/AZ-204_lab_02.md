...
1. 다음 명령을 실행하여 Azure Functions Core Tools를 사용하여 **dotnet 격리** 런타임을 사용하여 **현재 디렉터리에 새 로컬 Azure Functions 프로젝트를 만듭니**다.
...
1. **Echo.cs** 파일 내의 모든 내용을 삭제하고 다음 코드를 추가합니다. POST 요청을 받아서 요청 본문을 반환하는 "Echo"라는 HTTP 트리거 함수를 정의합니다. ILogger 인터페이스는 정보 로깅에도 사용됩니다. 이 함수는 HTTP 요청 및 응답을 테스트하는 데 사용할 수 있습니다.
...
1. 다음 명령을 실행하여 HTTP 요청 본문이 숫자 값 **3**으로 설정된 상태에서 POST** REST API 호출 `http://localhost:7071/api/echo` 테스트를 **실행합니다.

   ```powershell
   curl -X POST -i http://localhost:7071/api/echo -d 3
   ```

1. 다음 명령을 실행하여 HTTP** 요청 본문이 **숫자 값 **5**로 설정된 POST REST API 호출 `http://localhost:7071/api/echo` 을 테스트합니다.

   ```powershell
   curl -X POST -i http://localhost:7071/api/echo -d 5
   ```

1. 다음 명령을 실행하여 HTTP 요청 본문이 **Hello**의 **문자열 값으로 설정된 POST** REST API 호출 `http://localhost:7071/api/echo` 을 테스트합니다.

   ```powershell
   curl -X POST -i http://localhost:7071/api/echo -d "Hello"
   ```

1. 다음 명령을 실행하여 HTTP 요청 본문이 {"msg": "성공"}**의 JSON(JavaScript Object Notation) 값으로 설정된 POST** REST API 호출 `http://localhost:7071/api/echo` 을 테스트**** 합니다.
...
1. 줄 17에서 되풀이 빈도 간격을 5분이 아닌 2분으로 설정하고 변경 내용을 저장하려면 '"0 */2 * * *"''로 바꿉 `"0 */5 * * * *"` 니다.
...
1. 약 2분마다 발생하는 함수 실행을 관찰합니다. 각 함수 실행은 로그에 간단한 메시지를 렌더링해야 합니다.
1. 현재 실행 중인 **Windows 터미널** 애플리케이션을 닫습니다.
...
#### 작업 3: Blob 입력을 사용하여 HTTP 트리거 함수 코드 작성

1. **시작** 화면에서 **Visual Studio Code** 타일을 선택합니다.
1. **파일** 메뉴에서 **폴더 열기**를 선택합니다.
1. 열리는 **파일 탐색기** 창에서 **Allfiles (F):\\Allfiles\\Labs\\02\\Starter\\func**을 찾아본 후, **폴더 선택**을 선택합니다.
1. **Visual Studio Code** 창의 **탐색기** 창에서 **GetSettingInfo.cs** 파일을 엽니다.
1. 17줄부터 GetSettingInfo** 함수**의 자동 생성 코드 내용을 다음 코드로 바꿉니다.
...
1. 다음 콘텐츠로 구성되어야 하는 결과 코드를 검토합니다(이 코드는 HTTP GET 요청에 대한 응답으로 Azure Blob Storage 컨테이너에서 JSON 파일의 콘텐츠를 반환합니다.)
...
1. 명령 프롬프트에서 다음 명령을 실행하여 Microsoft.Azure.Functions.Worker.Extensions.Storage[ 확장을 등록](https://www.nuget.org/packages/Microsoft.Azure.Functions.Worker.Extensions.Storage/6.2.0)합니다.
...
1. 작업 표시줄에서 **Windows 터미널** 아이콘을 다시 선택하여 **Windows 터미널** 애플리케이션의 새 인스턴스를 엽니다.
1. 명령 프롬프트에서 다음 명령을 실행하여 HTTP 요청 본문이 숫자 값 **3**으로 설정된 POST** REST API 호출 `http://localhost:7071/api/echo` 을 테스트**합니다.
