...
1. **새 컨테이너** 팝업 창에서 다음 작업을 수행한 다음, **만들기** 를 선택합니다.
    ...
    | **퍼블릭 액세스 수준** 드롭다운 목록 | **프라이빗(익명 액세스 없음)** 을 선택합니다. |
...
1.  터미널에서 다음 명령을 실행하여 NuGet에서 **Azure.Storage.Blobs** 버전 12.0.0을 가져옵니다.
...
1.  이전에 이 랩에서 기록한 **스토리지 계정** 의 키로 값을 설정하여 **storageAccountKey** 문자열 상수를 업데이트합니다.
