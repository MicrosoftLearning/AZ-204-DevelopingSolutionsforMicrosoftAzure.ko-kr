...
1. 다음 명령을 실행하여 .NET 8.0을 사용하여 현재 디렉터리에 새 .NET 콘솔 애플리케이션을 만듭니다.

    ```powershell
    dotnet new console --output . --name ipcheck --framework net8.0
    ```

1. 다음 명령을 실행하여 Dockerfile이라는 **현재 디렉터리에 새 파일을 만듭니다**.
...
1. **Starter.sln** 파일을 삭제합니다.
    ...
    > **참고**: dotnet 명령을 사용할 때 .csproj 파일을 명시적으로 참조할 필요가 없습니다.

1. Windows 터미널** 애플리케이션으로 **전환합니다.
...
1. Visual Studio Code 창에서 **다시 전환하고 탐색기** 창에서 **Dockerfile** 파일을 엽니다**.**
...
1. **Dockerfile** 파일을 저장합니다.
    ...
    > **참고**: 다음으로 모든 프로젝트 파일을 포함하는 보관 파일을 만듭니다. 이렇게 하면 프로젝트 파일을 Cloud Shell에 업로드하는 것이 간소화됩니다.
...
1. **Cloud Shell** 창에서 업로드/다운로드 단추(오른쪽 아래 모서리의 반대 방향을 가리키는 두 개의 화살표가 있는 접힌 페이지로 표시됨)를 선택하고 드롭다운 메뉴에서 업로드**를 선택합니다**.

1. 열기 대화 상자에서 Allfiles(F):\\Allfiles\\Labs\\05\\Starter** 빈 디렉터리로 이동하고 **lab05.zip** 선택한 **다음 열기**를 선택합니다**.** ** 

    > **참고**: Cloud Shell** 세션 내**의 **/~** 디렉터리에 파일을 업로드합니다.

1. 포털의 **Cloud Shell** 명령 프롬프트에서 다음 명령을 실행하여 디렉터리에 ip검사****\~** 라는 **새 디렉터리를 만듭니다.

    ```bash
    mkdir ~/ipcheck
    ```

1. 다음 명령을 실행하여 새로 만든 디렉터리 **\~/ip검사** 업로드된 보관 파일의 콘텐츠를 추출합니다.

    ```bash
    unzip ~/lab05.zip -d ~/ipcheck
    ```

1. 다음 명령을 실행하여 /ip검사** 디렉터리에서 추출된 전체 콘텐츠에 **\~대한 읽기 및 실행 권한을 설정합니다.

    ```bash
    chmod -R +xr ~/ipcheck
    ```

1. 다음 명령을 실행하여 현재 디렉터리를 **\~/ip검사**로 변경합니다.
...
1. 다음 명령을 실행하여 Active Directory****/**\~\~를 /ip검사**로 변경합니다.
...
1. **리포지**토리 섹션에서 ip검사** 컨테이너 이미지가 포함된 **리포지토리를 선택합니다.

1. 리포지토리 블레이드의 **설정** 섹션에서 액세스 키를** 선택합니다**.

1. 액세스 키 블레이드에서 **관리 사용자** 검사 상자를 선택합니다**.**

1. 리포지토리 블레이드의 **서비스** 섹션에서 리포지**토리를 선택합니다**. 

1. 리포지토리 목록에서 ip검사** 선택합니다**.

1. **ip검사** 창에서 최신** 태그 항목과 **연결된 줄임표 메뉴를 선택한 다음 인스턴스 실행을** 선택합니다**.
