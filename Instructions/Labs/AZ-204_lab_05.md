...
1. **Cloud Shell** 창에서 업로드/다운로드 단추(오른쪽 하단에 반대 방향을 가리키는 두 개의 화살표가 있는 접힌 페이지로 표시됨)을 선택하고 드롭다운 메뉴에서 **업로드**를 선택합니다.
...
1. 리포지토리 블레이드의 **설정** 섹션에서 **액세스 키**를 선택합니다.

1. **액세스 키** 블레이드에서 **관리 사용자** 확인란을 선택합니다.

1. 리포지토리 블레이드의 **서비스** 섹션에서 **리포지토리**를 선택합니다. 

1. 리포지토리 목록에서 **ipcheck**를 선택합니다.
