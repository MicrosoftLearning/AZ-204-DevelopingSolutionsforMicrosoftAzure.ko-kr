...
## 로컬 랩 환경에 적절한 시간 설정이 있는지 확인합니다(표준 시간대에 따라 지연 없음).
...
이 랩에서는 Azure AD에서 애플리케이션을 등록하고, 사용자를 추가한 다음, 애플리케이션에 대한 사용자의 액세스를 테스트하여 Azure AD가 애플리케이션을 보호할 수 있는지 확인합니다. 또한 Graph 탐색기 도구를 사용하여 Azure AD 사용자 계정에 대한 Graph API에 대한 요청을 빌드하고 테스트합니다.
