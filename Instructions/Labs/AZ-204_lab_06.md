...
이 랩에서는 Microsoft Entra ID에 애플리케이션을 등록하고, 사용자를 추가한 다음, 애플리케이션에 대한 사용자의 액세스를 테스트하여 Entra ID가 이를 보호할 수 있는지 확인합니다. 또한 Graph 탐색기 도구를 사용하여 Entra ID 사용자 계정에 대한 Graph API에 대한 요청을 빌드하고 테스트합니다.
...
#### 작업 2: Entra ID 디렉터리에 애플리케이션 등록

1. Azure Portal에서 리소스, 서비스 및 문서 검색 텍스트 상자를 사용하여 **Entra ID**를 검색**하고 결과 목록에서 Microsoft Entra ID**를 선택합니다**.**

    > **참고**: 브라우저 세션을 Azure 구독과 연결된 Microsoft Entra ID 테넌트 블레이드로 리디렉션합니다.

1. Microsoft Entra ID **블레이드의**관리** 섹션에서 앱 등록 **** 선택합니다**. 
...
1. 다음 명령을 실행하여 기본 DNS(Do기본 이름 시스템)를 검색하고 표시합니다기본 Microsoft Entra 테넌트 이름입니다.
...
1. 다음 명령을 실행하여 새로 만든 Microsoft Entra ID 사용자의 UPN(사용자 계정 이름)을 식별합니다.
...
1. 다음 명령을 실행하여 모델 뷰 컨트롤러(MVC) 템플릿에 기반한 새 .NET Core 웹앱을 만듭니다(자리 표시자 `<application_ID>`, `<tenant_ID>`, `<domain_Name>`을 이 랩의 앞부분에서 기록한 값으로 바꿈).
    ...
    > **참고**: 터미널에서 오류가 발생한 경우 PowerShell을 사용할 수 있습니다. 이 경우 플래그를 `/S /Q` 제거하고 명령을 다시 실행합니다.
...
1. 요소 값 `<TargetFramework>` 이 net8.0으로 **설정되어 있는지 확인합니다**.

1. NuGet `Microsoft.AspNetCore.Authentication.OpenIdConnect` 패키지 버전이 `Microsoft.AspNetCore.Authentication.JwtBearer` 8.0.0**으로 **설정되어 있는지 확인합니다.
...
1. 루트 폴더에서 파일 **appsettings.json** 열고 다음 요소를 포함하여 AzureAd** 개체의 **콘텐츠를 검토합니다.
...
1. 열려 있는 브라우저 창에서 메시지가 표시되면 이 랩의 앞부분에서 만든 aad_lab_user1** Microsoft Entra ID 계정의 **UPN을 **사용하여 Pa55w.rd1234**를 암호로 사용하여 인증합니다.

    > **참고**: **계정 보호 도움말** 창이 표시되면 **지금은 건너뛰기**를 선택합니다.

    > **참고**: 관리자 승인**이 필요하다는 메시지가 **표시되면 관리자 권한이 있는 Microsoft Entra ID를 대신 사용하여 이 작업의 다시 기본der를 완료해야 합니다. 이렇게 하려면 관리자 계정이 있는 링크를 **선택하세요. 해당 계정**으로 로그인하고, 관리자 계정으로 로그인하고, 다음 단계를 진행합니다.
...
1. 브라우저에 **표시되는 대상 사이트의 시작** 홈 페이지를 검토하고 aad_user1** Microsoft Entra ID 계정의 **UPN이 브라우저 창에 표시되는지 확인합니다.
