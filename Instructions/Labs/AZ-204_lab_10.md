...
이 랩에서는 Azure Service Bus 큐를 사용하여 이 시나리오에 대한 개념 증명을 만듭니다. 시스템이 작동하는 방법을 보여 주려면 시스템에 메시지를 게시하는 .NET 프로젝트와 큐에서 메시지를 읽는 두 번째 .NET 애플리케이션을 만듭니다. 첫 번째 앱은 센서에서 생성되는 데이터를 시뮬레이션하고, 두 번째 앱은 처리를 위해 큐에서 메시지를 읽는 시스템을 시뮬레이션합니다.
...
1. 다음 코드를 추가합니다.
    ...
    ```
    > **참고**: **Service Bus 네임스페이스의 기본 커넥트ion 문자열로 값을 **설정하여 serviceBus커넥트ionString** 문자열** 상수 업데이트 및 queueName** 상수가 messagequeue**로 **설정되어 있는지 **확인합니다. 이 연습의 앞부분에서 만든 Service Bus 큐의 이름과 일치합니다.
...
1. Service Bus 탐색기를** 선택합니다**.

1. **피킹 모드** 탭 머리글을 선택한 **상태에서 큐** 탭에서 시작**에서 피킹 단추를 선택합니다**.
