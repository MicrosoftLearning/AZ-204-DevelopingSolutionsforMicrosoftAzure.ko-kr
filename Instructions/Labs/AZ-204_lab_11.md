...
1. **스케일 아웃 방법**이 표시될 때까지 아래로 스크롤합니다. **규칙 기반** 옵션을 클릭한 다음 **규칙 기반 크기 조정 관리**를 선택합니다.
