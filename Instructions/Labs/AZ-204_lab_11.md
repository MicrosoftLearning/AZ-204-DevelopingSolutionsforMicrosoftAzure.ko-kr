...
1. **구성** 섹션에서 다음 작업을 수행합니다.
    ...
    b.  APPLICATIONINSIGHTS_CONNECTION_STRING** 키를 나타내는 **값을 확인합니다. 이 값은 웹 API 리소스를 빌드할 때 자동으로 설정되었습니다.
...
1. Scale Out 메서드**가 **표시될 때까지 아래로 스크롤합니다. **규칙 기반** 옵션을 클릭한 다음 규칙 기반 크기 조정** 관리를 선택합니다**.
...
1. 편집기** 창에서 **17줄에서 다음 코드를 찾아 삭제합니다.
...
1. **Program.cs** 파일에서 Application Insights와 함께 Service 컬렉션에 등록하여 Profiler를 사용하도록 설정하려면 줄 7부터 다음 두 줄을 추가합니다.
...
1. 브라우저 창이 열리면 `http://localhost` URL로 이동하여 웹 API의 **/weatherforecast** 상대 경로를 추가합니다.
    ...
    > **참고**: 전체 URL은 `http://localhost:[port-number]/weatherforecast`입니다. 여기서 `[port-number]` 자리 표시자는 HTTPS 프로토콜을 통해 웹앱에 액세스할 수 있는 포트 번호를 식별합니다.
...
1. Visual Studio Code 창의 ****탐색기** 창에서 bin\Debug\net8.0** 디렉터리로 이동합니다**.**
    ...
    > **참고**: bin\Debug\net8.0** 디렉터리에 있는지 **확인하려면 Visual Studio Code의 파일 메뉴에서 폴더 열기를 선택하고 Allfiles(F):\\Allfiles\\Labs\\11\\Starter\\Api\\bin\\Release\\net8.0\win-x86\publish**로 이동**한 다음 폴더** 선택을 선택합니다**.

1. **Visual Studio Code** 창의 메뉴 모음에서 **터미널**을 선택한 다음, **새 터미널**을 선택합니다.

1. 터미널 프롬프트에서 다음 명령을 실행하여 현재 디렉터리가 Allfiles(F):Allfiles\\Labs\\11\\Starter\\Api\\bin\\Release\\net8.0 net8.0\\\\win-x86\\publish**로 설정되어 **있는지 확인합니다. 여기서 배포 파일이 상주합니다.\\
...
1. 메시지가 표시되면 이 랩에 사용 중인 Azure 구독에 액세스하기 위한 자격 증명을 제공하여 인증합니다.
