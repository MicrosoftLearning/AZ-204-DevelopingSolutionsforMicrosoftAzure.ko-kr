...
1. **스토리지 계정** 창의 **보안 + 네트워킹** 섹션에서 **액세스 키** 링크를 선택합니다.
...
1. **비밀 만들기** 창에서 다음 작업을 수행한 후, **만들기** 를 선택합니다.
    ...
    | **콘텐츠 형식** 텍스트 상자 | 비워 둡니다. |
    | **활성화 날짜 설정** 확인란 | 선택하지 않음 |
    | **만료 날짜 설정** 확인란 | 선택하지 않음 |
    | **사용** 옵션 | **예** 를 선택합니다. |
...
1. **값** 텍스트 상자에서 다음 구문을 사용하여 값을 생성합니다. ``@Microsoft.KeyVault(SecretUri=*Secret Identifier*)``

    > **참고**: 위의 구문을 사용하여 **_비밀 식별자_** 에 대한 참조를 빌드해야 합니다. 예를 들어, 비밀 식별자가 `https://securevaultstudent.vault.azure.net/secrets/storagecredentials/17b41386df3e4191b92f089f5efb4cbf`인 경우 값은 `@Microsoft.KeyVault(SecretUri=https://securevaultstudent.vault.azure.net/secrets/storagecredentials/17b41386df3e4191b92f089f5efb4cbf)`가 됩니다.
...
1. 표시되는 팝업 대화 상자의 **HTTP 메서드** 목록에서 **GET** 을 선택합니다.
...
1. 다음 명령을 실행하여 NuGet에서 **Azure.Storage.Blobs** 패키지의 **12.6.0** 버전을 추가합니다.
...
1. 다음 명령을 실행하여 함수 앱 프로젝트를 다시 게시합니다.
