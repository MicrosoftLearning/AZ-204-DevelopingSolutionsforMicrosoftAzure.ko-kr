...
1. 키 자격** 증명 모음 만들기 블레이드의 **기본 사항** 탭에서 다음 작업을 수행한 다음, Access Configuration** 탭을 선택합니다**.**
...
1. 액세스 구성 탭에서 **권한 모델을 자격 증명 모음 액세스 정책**으로 **변경**합니다.

1. 검토 + 만들기** 탭을 **선택하고 이전 단계에서 선택한 옵션을 검토합니다.
...
1. **액세스 정책** 만들기 블레이드의 **Permissons** 섹션에서 다음을 선택합니다.
...
1. 코드 편집기에서 18줄부터 함수 FileParser**의 **자동 생성 코드 내용을 다음 코드로 바꿉니다.
...
1. 다음 콘텐츠로 구성되어야 하는 결과 코드를 검토합니다(이 코드는 Storage커넥트ionString 환경 변수의 값을 반환합니다.)
...
1. 다음 명령을 실행하여 GET** REST API 호출에 대한 테스트를 **실행합니다`http://localhost:7071/api/FileParser`.
...
1. **Microsoft Edge** 브라우저 창에서 Azure 구독에 액세스하는 데 사용하는 계정의 자격 증명을 입력한 다음, 로그인을 선택합니다.****
...
1. **개요 페이지의 맨 아래에 있는 함수** 탭에서 기존 FileParser** 함수를 **선택합니다.
...
1. 다음 명령을 실행하여 NuGet에서 Azure.Storage.Blobs 패키지 버전 **12.18.0**을 추가**합니다**.
...
1. Azure.Storage.Blobs** 및 Azure.Storage.Blobs.Models**** 네임스페이스에 대한 **using 지시문을 추가**** 합니다.
...
1. 19줄부터 FileParser** 함수**의 자동 생성된 코드 내용을 다음 코드로 바꿉니다.
...
1. 다음 콘텐츠로 구성되어야 하는 결과 코드를 검토합니다(이 코드는 Storage커넥트ionString 환경 변수로 지정된 스토리지 계정에서 다운로드한 Blob의 콘텐츠를 반환합니다.)
...
1. **개요 페이지의 함수** 탭에서 기존 FileParser** 함수를 **선택합니다.
